...
<body>
	...
	<div class="terminal">
		<h1>
			Oops! <span class="errorcode">Not Available!</span>
		</h1>
		<p class="output">The page you are looking for might have been
			removed, had its name changed or is temporarily unavailable.</p>
		<p class="output">Get in touch with the site owner!</p>
		<p class="output">
			<a href="#1" onclick="history.back();">go back</a> or <a href="main">Main
				Page</a>.
		</p>
		${status}<br /> ${error}<br /> ${timestamp }<br />

		<!-- 무슨 에러인지 표시
		<p>경로: ${path }</p> 어디서 오류가 났는지
		<p>오류: ${error}</p> 왜 에러가 나는지
		<p>메시지: ${message}</p> 
		<p>상태: ${status}</p> 404 or 500 or..
		<p>발생 시간: ${timestamp}</p>
